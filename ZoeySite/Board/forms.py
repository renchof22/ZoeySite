...
class TopicForm(forms.ModelForm):
    # text_tag = forms.CharField(max_length=160, required=False, label='タグ：')
    ...
    image = forms.ImageField(widget=FileInputWithPreview(include_preview=False), required=False)

    class Meta:
        model = Topic
        fields = ('contents', 'image', 'tags')

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # 画像投稿<input>を装飾するため
        self.fields['image'].widget.attrs['style'] = "display:none"
        # self.fields['text_tag'].widget.attrs['placeholder'] = 'スペース区切り'
        # self.fields['text_tag'].widget.attrs['class'] = 'text-tag'
